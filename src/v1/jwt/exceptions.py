...
expired_token_exception = HTTPException(
    status_code=status.HTTP_401_UNAUTHORIZED, detail=f"Просроченный токен!"
)
forbidden_admin_available_exception = HTTPException(
    status_code=status.HTTP_403_FORBIDDEN, detail=f"Вы не являетесь админом сервиса!"
)
forbidden_stuff_available_exception = HTTPException(
    status_code=status.HTTP_403_FORBIDDEN, detail=f"Вы не являетесь работником сервиса!"
)
